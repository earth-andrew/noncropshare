...
to calculate-score
  ...
  foreach playerPosition [ ?1 ->
    ...
    let tempBonuses (sum [currentNCHBonus] of patches with [player = ?1])
    let tempSpending (sum [currentSpending] of patches with [player = ?1])
      ...
      if ?1 != player [
        ...
        ask crops-here [
          ifelse previousChoice = 0 [
            hubnet-send-override  (item (position ?1 playerPosition) playerNames) self "shape" ["crop-base"]
          ] [ ifelse previousChoice = 1 [

            hubnet-send-override  (item (position ?1 playerPosition) playerNames) self "shape" ["non-crop"]

          ] [ ifelse previousChoice = 2 [
            hubnet-send-override  (item (position ?1 playerPosition) playerNames) self "shape" ["crop-spray-light"]

          ] [

          hubnet-send-override  (item (position ?1 playerPosition) playerNames) self "shape" ["crop-spray-heavy"]
